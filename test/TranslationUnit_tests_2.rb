...
class TestTranslationUnitCreation < MiniTest::Test
  ...
  def test_create_TU_from_source_file
    tu = @cindex.translation_unit(source: SOURCE_FILE, args: CLANG_HEADERS_PATH)
    assert_instance_of Clangc::TranslationUnit, tu 
  end
  
  def test_fail_create_TU_from_source_file
    tu = @cindex.translation_unit(source: BAD_FILE, args: CLANG_HEADERS_PATH)
    assert_nil tu 
  end
  ...
  def test_create_TU_from_source_file_in_cmdline
    tu = @cindex.translation_unit(args: [SOURCE_FILE] + CLANG_HEADERS_PATH)
    assert_instance_of Clangc::TranslationUnit, tu 
  end
  
  def test_fail_create_TU_from_source_file_in_cmdline
    tu = @cindex.translation_unit(args: [BAD_FILE] + CLANG_HEADERS_PATH)
    assert_nil tu 
  end
  
  def test_create_TU
    tu = @cindex.translation_unit(ast: AST_FILE)
    assert_instance_of Clangc::TranslationUnit, tu 
  end
  
  def test_fail_create_TU
    tu = @cindex.translation_unit(ast: BAD_FILE)
    assert_nil tu 
  end
  
  # return an error code if the Translation Unit creation fail
  def test_create_TU2
    tu = @cindex.translation_unit(ast: AST_FILE, error: true)
    assert_instance_of Clangc::TranslationUnit, tu 
  end
  
  def test_fail_create_TU2
    tu = @cindex.translation_unit(ast: BAD_FILE, error: true)
  ...
  def test_parse_TU
    ...
    tu = @cindex.translation_unit(parse: SOURCE_FILE,
  ...
  def test_fail_parse_TU
    options = Clangc::TranslationUnit_Flags::NONE
    tu = @cindex.translation_unit(parse: BAD_FILE,args: CLANG_HEADERS_PATH,
                                  flags: options)
    assert_nil tu
  end
  ...
  def test_parse_TU2
    options = Clangc::TranslationUnit_Flags::NONE
    tu = @cindex.translation_unit(parse: SOURCE_FILE, args: CLANG_HEADERS_PATH,
