...
class TestTypeUsage < MiniTest::Test
  ...
  def test_type_pointee_type
    tu = @cindex.create_translation_unit(source: SOURCE_FILE_POINTER,
                                         args: CLANG_HEADERS_PATH)
    Clangc.visit_children(cursor: tu.cursor) do |cursor, parent|
      ...
      if cursor.type.kind == Clangc::TypeKind::POINTER
        assert_equal(Clangc::TypeKind::INT,
                     cursor_pointee.kind,
                     cursor_pointee.spelling)
      else
        assert_equal(Clangc::TypeKind::INVALID,
                     cursor_pointee.kind,
                     cursor_pointee.spelling)
      end
      Clangc::ChildVisitResult::RECURSE
  ...
  def test_type_constant_qualified_type
    tu = @cindex.create_translation_unit(source: SOURCE_FILE_QUALIFIED,
                                         args: CLANG_HEADERS_PATH)
  ...
  def test_type_volatile_qualified_type
    ...
    Clangc.visit_children(cursor: tu.cursor) do |cursor, _parent|
      ...
      if location[1] == 3 && location[2] == 14
        assert(cursor.type.is_volatile_qualified, cursor.location.spelling[2])
      else
        assert(!cursor.type.is_volatile_qualified, cursor.location.spelling[2])
      end
      Clangc::ChildVisitResult::RECURSE
  ...
  def test_type_restrict_qualified_type
    ...
    Clangc.visit_children(cursor: tu.cursor) do |cursor, _parent|
      ...
      if location[1] == 2 && location[2] == 15
        assert(cursor.type.is_restrict_qualified, location[2])
      else
        assert(!cursor.type.is_restrict_qualified, location[2])
      end
      Clangc::ChildVisitResult::RECURSE
  ...
  def test_type_function_return_type
    tu = @cindex.create_translation_unit(source: SOURCE_FILE_FUNCTION,
                                         args: CLANG_HEADERS_PATH)
    Clangc.visit_children(cursor: tu.cursor) do |cursor, _parent|
  ...
  def test_type_function_calling_conv
    ...
    Clangc.visit_children(cursor: tu.cursor) do |cursor, _parent|
      ...
      if cursor_kind == Clangc::TypeKind::FUNCTION_NO_PROTO ||
         cursor_kind == Clangc::TypeKind::FUNCTION_PROTO
        convention_found = false
        Clangc::CallingConv.constants.each do |c|
          if Clangc::CallingConv.const_get(c) == cursor.type.calling_conv
  ...
  def test_type_function_num_arg_types
    ...
    Clangc.visit_children(cursor: tu.cursor) do |cursor, _parent|
      ck = cursor.type.kind
      if ck == Clangc::TypeKind::FUNCTION_PROTO ||
         ck == Clangc::TypeKind::FUNCTION_NO_PROTO
  ...
  def test_type_function_arg_type
    ...
    Clangc.visit_children(cursor: tu.cursor) do |cursor, _parent|
      ...
      if ck == Clangc::TypeKind::FUNCTION_PROTO ||
        ck == Clangc::TypeKind::FUNCTION_NO_PROTO
        assert_equal(Clangc::TypeKind::CHAR_S,
                     cursor.type.arg_type(0).kind,
                     cursor.type.arg_type(0).kind)
        assert_equal(Clangc::TypeKind::INT,
                     cursor.type.arg_type(1).kind,
                     cursor.type.arg_type(1).kind)
      end
      Clangc::ChildVisitResult::RECURSE
    end
  ...
  def test_type_element_type
    tu = @cindex.create_translation_unit(source: SOURCE_FILE_ARRAY,
                                         args: CLANG_HEADERS_PATH)
    Clangc.visit_children(cursor: tu.cursor) do |cursor, _parent|
      if cursor.type.kind != Clangc::TypeKind::INVALID &&
         cursor.type.kind != Clangc::TypeKind::INT
        assert_equal(Clangc::TypeKind::CONSTANT_ARRAY,
                     cursor.type.kind,
                     cursor.location.spelling.inspect)
        assert_equal(Clangc::TypeKind::INT,
                     cursor.type.element_type.kind,
                     cursor.location.spelling.inspect)
      elsif cursor.type.kind != Clangc::TypeKind::INVALID &&
            cursor.type.kind != Clangc::TypeKind::INT
        assert_equal(Clangc::TypeKind::VECTOR,
                     cursor.type.kind,
                     cursor.location.spelling.inspect)
        assert_equal(Clangc::TypeKind::INT,
                     cursor.type.element_type.kind,
    ...
    Clangc.visit_children(cursor: tu.cursor) do |cursor, _parent|
      if cursor.type.kind != Clangc::TypeKind::INVALID &&
        ...
        assert_equal(Clangc::TypeKind::CONSTANT_ARRAY,
                     cursor.type.kind,
                     cursor.location.spelling.inspect)
        assert_equal(10, cursor.type.num_elements,
                     cursor.location.spelling.inspect)
      elsif cursor.type.kind != Clangc::TypeKind::INVALID &&
            cursor.type.kind != Clangc::TypeKind::INT
        assert_equal(Clangc::TypeKind::VECTOR,
                     cursor.type.kind,
                     cursor.location.spelling.inspect)
        assert_equal(2, cursor.type.num_elements,
                     cursor.location.spelling.inspect)
      end
      Clangc::ChildVisitResult::RECURSE
    end
    ...
    Clangc.visit_children(cursor: tu.cursor) do |cursor, _parent|
      if cursor.type.kind != Clangc::TypeKind::INVALID &&
        ...
        assert_equal(Clangc::TypeKind::CONSTANT_ARRAY,
                     ...
                     cursor.location.spelling.inspect)
        assert_equal(Clangc::TypeKind::INT,
                     cursor.type.array_element_type.kind,
                     cursor.location.spelling.inspect)
      end
      Clangc::ChildVisitResult::RECURSE
    end
    ...
    Clangc.visit_children(cursor: tu.cursor) do |cursor, _parent|
      if cursor.type.kind != Clangc::TypeKind::INVALID &&
        ...
        assert_equal(Clangc::TypeKind::CONSTANT_ARRAY,
                     cursor.type.kind,
                     cursor.location.spelling.inspect)
        assert_equal(10, cursor.type.array_size,
                     cursor.location.spelling.inspect)
      end
      Clangc::ChildVisitResult::RECURSE
    end
  ...
  def test_type_is_pod
    ...
    tu = @cindex.create_translation_unit(source: SOURCE_FILE_POD,
                                         args: arguments)
    Clangc.visit_children(cursor: tu.cursor) do |cursor, _parent|
      # ensure this is not an included source
      if cursor.location.spelling[0].name == SOURCE_FILE_POD
        if cursor.type.kind == Clangc::TypeKind::INT
          assert_equal(true, cursor.type.is_pod,
                       cursor.location.spelling.inspect + cursor.spelling)
        elsif cursor.type.kind != Clangc::TypeKind::INVALID &&
              cursor.type.kind != Clangc::TypeKind::INT
          assert_equal(false, cursor.type.is_pod,
                       cursor.location.spelling.inspect + cursor.spelling)
        end
      end
      Clangc::ChildVisitResult::RECURSE
  ...
  def test_type_align_of
    arguments = ["-x", "c++", "--std", "c++11"] + CLANG_HEADERS_PATH
    tu = @cindex.create_translation_unit(source: SOURCE_FILE_ALIGN_OF,
                                         args: arguments)
    Clangc.visit_children(cursor: tu.cursor) do |cursor, _parent|
      if cursor.location.spelling[0].name == SOURCE_FILE_ALIGN_OF
        case cursor.spelling
        when "toto"
          assert_equal(Fiddle::ALIGN_CHAR, cursor.type.align_of,
                       cursor.type.align_of)
        when "a"
          assert_equal(Fiddle::ALIGN_INTPTR_T, cursor.type.align_of,
                       cursor.type.align_of)
        end
      end
      Clangc::ChildVisitResult::RECURSE
  ...
  def test_type_size_of
    ...
    Clangc.visit_children(cursor: tu.cursor) do |cursor, _parent|
      if cursor.location.spelling[0].name == SOURCE_FILE_ALIGN_OF
        case cursor.spelling
        when "toto"
          assert_equal(Fiddle::SIZEOF_CHAR, cursor.type.size_of,
                       cursor.type.size_of)
        when "a"
          assert_equal(Fiddle::SIZEOF_INTPTR_T, cursor.type.size_of,
                       cursor.type.size_of)
        end
      end
      Clangc::ChildVisitResult::RECURSE
  ...
  def test_type_get_offset_of
    arguments = ["-x", "c++", "--std", "c++11"] + CLANG_HEADERS_PATH
    tu = @cindex.create_translation_unit(source: SOURCE_FILE_TYPE_OFFSET_OF,
                                         args: arguments)
    Clangc.visit_children(cursor: tu.cursor) do |cursor, _parent|
      if cursor.location.spelling[0].name == SOURCE_FILE_TYPE_OFFSET_OF
        if cursor.kind == Clangc::CursorKind::STRUCT_DECL
          assert_equal(Fiddle::SIZEOF_INT * 8, cursor.type.offset_of("f"),
                       cursor.kind)
          assert_equal(Fiddle::SIZEOF_FLOAT * 8 + Fiddle::SIZEOF_INT * 8,
                       cursor.type.offset_of("c"), cursor.kind)
        else
          assert_equal(Clangc::TypeLayoutError::INVALID,
                       cursor.type.offset_of("f"), cursor.kind)
        end
      end
      Clangc::ChildVisitResult::RECURSE
    end
  ...
  def test_type_get_num_template_arguments
    arguments = ["-x", "c++", "--std", "c++11"] + CLANG_HEADERS_PATH
    source = SOURCE_FILE_CLASS_TEMPLATE_SPECIALISATION
    tu = @cindex.create_translation_unit(source: source,
                                         args: arguments)
    Clangc.visit_children(cursor: tu.cursor) do |cursor, _parent|
      if cursor.location.spelling[0].name == source
        type = cursor.type
        if cursor.kind == Clangc::CursorKind::CLASS_DECL
          assert_equal(1, type.num_template_arguments,
                       type.num_template_arguments)
        else
          assert_equal(-1, type.num_template_arguments,
                       type.num_template_arguments)
        end
    ...
    Clangc.visit_children(cursor: tu.cursor) do |cursor, _parent|
      if cursor.location.spelling[0].name == source
        if cursor.kind == Clangc::CursorKind::CLASS_DECL
          ...
          assert_equal(Clangc::TypeKind::INT, template_argument.kind,
  ...
  def test_type_get_cxx_ref_qualifier
    arguments = ["-x", "c++", "--std", "c++11"] + CLANG_HEADERS_PATH
    tu = @cindex.create_translation_unit(source: SOURCE_FILE_CXX_REF_QUALIFIER,
                                         args: arguments)
    Clangc.visit_children(cursor: tu.cursor) do |cursor, _parent|
      if cursor.location.spelling[0].name == SOURCE_FILE_CXX_REF_QUALIFIER
        if cursor.kind == Clangc::CursorKind::CXX_METHOD
          ...
          case line
          when 5
            assert_equal(Clangc::RefQualifierKind::L_VALUE,
                         type.cxx_ref_qualifier,
                         "#{cursor.spelling} #{line} #{type.cxx_ref_qualifier}")
          when 6
            assert_equal(Clangc::RefQualifierKind::R_VALUE,
                         type.cxx_ref_qualifier,
                         "#{cursor.spelling} #{line} #{type.cxx_ref_qualifier}")
          end
        end
